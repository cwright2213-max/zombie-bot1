"""Entry point for the Discord bot - FINAL LOCKED TO FREAK 572053060969299977"""
...
async def zombie_start(interaction: discord.Interaction) -> None:
    ...
    game_store.save()
    await interaction.response.send_message(embed=combat_embed(messages, player), view=CombatView(interaction.user.id, game_store))
...
async def zombie_status(interaction: discord.Interaction) -> None:
    await interaction.response.send_message(status(game_store.get(interaction.user.id)), view=ZombieMenuView(interaction.user.id, game_store))
...
async def zombie_action(interaction: discord.Interaction, action: app_commands.Choice[str], heal_item: app_commands.Choice[str] | None = None) -> None:
    player = game_store.get(interaction.user.id)
    messages = take_action(player, action.value, heal_item.value if heal_item else None)
    game_store.save()
    await interaction.response.send_message(embed=combat_embed(messages, player), view=CombatView(interaction.user.id, game_store) if player.run_active else ZombieMenuView(interaction.user.id, game_store))
...
@app_commands.choices(item=[app_commands.Choice(name="Ammo box", value="ammo"), app_commands.Choice(name="Painkillers", value="painkillers"), app_commands.Choice(name="Full restore", value="full_restore"), *[app_commands.Choice(name=weapon_name, value=weapon_name) for weapon_name in WEAPONS if weapon_name != "Pistol"]])
...
@app_commands.choices(zone_name=[app_commands.Choice(name=zone_name, value=zone_name) for zone_name in ("Graveyard","Mega Death City","Frostbitten Outskirts","Toxic Wasteland","The Void")])
...
@app_commands.choices(ammo_name=[app_commands.Choice(name=ammo_name, value=ammo_name) for ammo_name in AMMO])
...
@app_commands.choices(stat=[app_commands.Choice(name="Max health +20", value="health"), app_commands.Choice(name="Weapon damage +3", value="damage"), app_commands.Choice(name="Magazine size +1", value="mag"), app_commands.Choice(name="Crit chance +2%", value="crit"), app_commands.Choice(name="Armor -2 dmg", value="armor"), app_commands.Choice(name="Loot +5% money", value="scavenger")])
...
# --- ADMIN ONLY - LOCKED TO YOU 572053060969299977 ---
...
async def addmoney(interaction: discord.Interaction, amount: int):
    if not is_admin(interaction):
        await interaction.response.send_message(f"❌ Admin-only. Your ID is `{interaction.user.id}`", ephemeral=True)
        return
    player = game_store.get(interaction.user.id)
    player.money += amount
    game_store.save()
...
@bot.tree.command(name="setmoney", description="[ADMIN] Set money")
...
async def addstars(interaction: discord.Interaction, amount: int):
    ...
    player.stars += amount
    game_store.save()
    await interaction.response.send_message(f"⭐ **+{amount} stars** → {player.stars}", ephemeral=True)
